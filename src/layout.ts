...
const defaultOptions: Options = {
  ...
  doubleHyphenPenalty: 0,
  adjacentLooseTightPenalty: 0,
  widowPenalty: 800,        // conservative default; override per call if needed
  minLastLineRatio: 0.25,   // consider last line too short if <25% of measure
};
...
export function breakLines(
  ...
  // Choose active node with fewest total demerits as the last breakpoint,
  // with an optional terminal "widow" penalty for very short last lines.
  const paragraphTotalWidth = sumWidth;

  let bestNode: Node | null = null;
  let bestScore = Infinity;
  active.forEach((a) => {
    let score = a.totalDemerits;

    // Terminal widow control
    if (opts_.widowPenalty && opts_.minLastLineRatio != null) {
      const ideal = lineLen(a.line);
      const actual = paragraphTotalWidth - a.totalWidth; // width from this break to end
      if (ideal > 0) {
        const ratio = actual / ideal;
        if (ratio < opts_.minLastLineRatio) {
          const deficit = opts_.minLastLineRatio - ratio;
          score += opts_.widowPenalty * deficit * deficit; // quadratic scaling
        }
      }
    }
