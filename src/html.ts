 ...
 */
function measureWhitespaceRunDOM(
  el: Element,
  run: string,
  measureFn: (context: Element, word: string) => number
) {
...
function addItemsForTextNode(
  ...
  const chunks = text.split(/(\s+)/u).filter((w) => w.length > 0);
  let textOffset = 0;

  // Track previous non-space chunk for punctuation penalty
  ...
  chunks.forEach((w) => {
    if (isSpace(w)) {
      ...
      if (w.indexOf(NBSP) !== -1) {
        const width = measureWhitespaceRunDOM(el, w, measureFn);
        const box: DOMBox = {
          type: 'box',
          width,
          node,
      ...
      if (OTHER_UNICODE_SPACE_RE.test(w)) {
        const width = measureWhitespaceRunDOM(el, w, measureFn);
        const mult = spaceWidth > 0 ? width / spaceWidth : w.length;

        // Penalty if previous token ended with closing punctuation
        if (prevNonSpaceChunk && CLOSE_PUNCT_RE.test(prevNonSpaceChunk)) {
          const pen: DOMPenalty = {
            type: 'penalty',
            width: 0,
      ...
      // ASCII whitespace run (spaces/tabs/newlines) — browser collapses to one space
      // Penalize break after closing punctuation
      if (prevNonSpaceChunk && CLOSE_PUNCT_RE.test(prevNonSpaceChunk)) {
        const pen: DOMPenalty = {
          type: 'penalty',
          width: 0,
    ...
    // Discourage breaking before opening punctuation if the last item is a glue.
    if (OPEN_PUNCT_RE.test(w) && items.length >= 1 && items[items.length - 1].type === 'glue') {
      ...
      // Insert the penalty just before the glue
      items.splice(items.length - 1, 0, pen);
    }
...
function elementLineWidth(el: HTMLElement) {
  const { width, boxSizing, paddingLeft, paddingRight, borderLeftWidth, borderRightWidth } =
    getComputedStyle(el);
  let w = parseFloat(width!);
  if (boxSizing === 'border-box') {
    w -= parseFloat(paddingLeft!);
    w -= parseFloat(paddingRight!);
    w -= parseFloat(borderLeftWidth!);
    w -= parseFloat(borderRightWidth!);
  }
  return w;
}
...
// Hanging punctuation helpers
function wrapCharForHanging(t: Text, index: number, side: 'start' | 'end', px: number) {
  if (px <= 0) return;
  const before = t;
  const chNode = before.splitText(index);
  const after = chNode.splitText(1);
  ...
  if (side === 'start') {
    ...
    span.style.marginRight = `${-px}px`;
    span.style.paddingRight = `${px}px`;
  }
  chNode.parentNode!.insertBefore(span, chNode);
  span.appendChild(chNode);
}

function applyHangingPunctuation(r: Range, startPx = 2, endPx = 2) {
  const texts = textNodesInRange(r, isTextOrInlineElement);

  // Start of line: first non-space char
  outerStart: for (const t of texts) {
    const s = t.nodeValue || '';
    for (let i = 0; i < s.length; i++) {
      const ch = s[i];
      if (!/\s/.test(ch)) {
        if (START_HANG_RE.test(ch)) wrapCharForHanging(t, i, 'start', startPx);
        break outerStart;
      }
    }
  }

  // End of line: last non-space char
  outerEnd: for (let ti = texts.length - 1; ti >= 0; ti--) {
    const t = texts[ti];
    const s = t.nodeValue || '';
    for (let i = s.length - 1; i >= 0; i--) {
      const ch = s[i];
      if (!/\s/.test(ch)) {
        if (END_HANG_RE.test(ch)) wrapCharForHanging(t, i, 'end', endPx);
        break outerEnd;
      }
    }
  }
...
export function justifyContent(
  ...
  elements.forEach((el) => {
    const lineWidth = elementLineWidth(el);
    let items: DOMItem[] = [];
    // Prefer hyphenation on the first pass for smoother spacing
    addItemsForNode(items, el, measure, hyphenateFn);
    let breakpoints;
    try {
      breakpoints = breakLines(items, lineWidth, {
      ...
      if (e instanceof MaxAdjustmentExceededError) {
        // Retry with unlimited stretching (hyphenation kept on)
        breakpoints = breakLines(items, lineWidth, {
  ...
  elementBreaks.forEach(({ el, items, breakpoints, lineWidth }) => {
    ...
    // Apply subtle hanging punctuation before spacing adjustment
    lineRanges.forEach((r) => {
      applyHangingPunctuation(r, 2, 2); // 1–3 px is usually best on screen
    });

    // Adjust inter-word spacing on each line and add hyphenation if needed.
    lineRanges.forEach((r, i) => {
      const glueCount = glueCounts[i];
      if (glueCount === 0) {
        return; // No inter-word gaps — nothing to adjust
      }

      const spaceDiff = lineWidth - actualWidths[i];
      const extraSpacePerGlue = spaceDiff / glueCount;
